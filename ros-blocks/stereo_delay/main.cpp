...
//  delay_s = delay_ms/1000.0;
  ...
  image_transport::Subscriber sub_L = it.subscribe("stereo/slave/left/image_raw"  , 1, imageCallback_L);
  image_transport::Subscriber sub_R = it.subscribe("stereo/slave/right/image_raw" , 1, imageCallback_R);

  ros::Subscriber sub_Delay = nh.subscribe("dvrk/console/teleop/delay"   , 1, delayCallback);


  //set up publisher:
  image_transport::Publisher pub_L = it.advertise("stereo/master/left/image_raw" , 1);
  image_transport::Publisher pub_R = it.advertise("stereo/master/right/image_raw", 1);
