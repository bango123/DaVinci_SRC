...
int svlVideoCodecFFMPEG::DialogCompression(const std::string &filename)
{
    ...
    if (extension == "mpg" || extension == "mpeg") {
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_MPEG1VIDEO);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_MPEG2VIDEO);
    }
    else if (extension == "avi") {
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_MPEG4);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_MSMPEG4V2);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_MSMPEG4V3);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_DVVIDEO);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_MJPEG);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_HUFFYUV);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_FFV1);
...
//      encoder_list[encoder_count ++] = GetFFMPEGEncoderID(CODEC_ID_TIFF);
        ...
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_THEORA);
    }
    else if (extension == "dv") {
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_DVVIDEO);
    }
    else if (extension == "wmv") {
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_WMV1);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_WMV2);
    }
    else if (extension == "flv") {
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_FLV1);
...
//      encoder_list[encoder_count ++] = GetFFMPEGEncoderID(CODEC_ID_RV20);
        ...
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_ASV1);
        encoder_list[encoder_count ++] = GetFFMPEGEncoderID(AV_CODEC_ID_ASV2);
    }
...
void svlVideoCodecFFMPEG::GetEncoderList(std::string & encoder_list) const
{
    ...
    if (extension.empty()) {
        ...
        if (extension == "mpg" || extension == "mpeg") {
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_MPEG1VIDEO); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_MPEG2VIDEO); strstr << encid << ":" << EncoderNames[encid] << "\n";
        }
        else if (extension == "avi") {
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_MPEG4); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_MSMPEG4V2); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_MSMPEG4V3); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_DVVIDEO); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_MJPEG); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_HUFFYUV); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_FFV1); strstr << encid << ":" << EncoderNames[encid] << "\n";
...
//          encid = GetFFMPEGEncoderID(CODEC_ID_TIFF); strstr << encid << ":" << EncoderNames[encid] << "\n";
            ...
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_THEORA); strstr << encid << ":" << EncoderNames[encid] << "\n";
        }
        else if (extension == "dv") {
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_DVVIDEO); strstr << encid << ":" << EncoderNames[encid] << "\n";
        }
        else if (extension == "wmv") {
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_WMV1); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_WMV2); strstr << encid << ":" << EncoderNames[encid] << "\n";
        }
        else if (extension == "flv") {
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_FLV1); strstr << encid << ":" << EncoderNames[encid] << "\n";
...
//          encid = GetFFMPEGEncoderID(CODEC_ID_RV20); strstr << encid << ":" << EncoderNames[encid] << "\n";
            ...
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_ASV1); strstr << encid << ":" << EncoderNames[encid] << "\n";
            encid = GetFFMPEGEncoderID(AV_CODEC_ID_ASV2); strstr << encid << ":" << EncoderNames[encid] << "\n";
        }
        else if (extension == "roq") {
...
void svlVideoCodecFFMPEG::ConfigureEncoder()
{
    ...
    else if (codec_id == AV_CODEC_ID_JPEGLS) {
...
std::string svlVideoCodecFFMPEG::GetExtensionFromEncoderID(unsigned int encoder_id)
{
    ...
    switch (EncoderIDs[encoder_id]) {
        case AV_CODEC_ID_MPEG1VIDEO:   // MPEG-1 Video
        case AV_CODEC_ID_MPEG2VIDEO:   // MPEG-2 Video
            extension = "mpg";
        break;

        case AV_CODEC_ID_H261:         // H.261
        case AV_CODEC_ID_H263:         // H.263 / H.263-1996
        case AV_CODEC_ID_MJPEG:        // Motion JPEG
        case AV_CODEC_ID_LJPEG:        // Lossless JPEG
        case AV_CODEC_ID_JPEGLS:       // JPEG-LS
        case AV_CODEC_ID_MPEG4:        // MPEG-4 part 2
        case AV_CODEC_ID_RAWVIDEO:     // Raw video
        case AV_CODEC_ID_MSMPEG4V1:    // MPEG-4 part 2 Microsoft variant version 1
        case AV_CODEC_ID_MSMPEG4V2:    // MPEG-4 part 2 Microsoft variant version 2
        case AV_CODEC_ID_MSMPEG4V3:    // MPEG-4 part 2 Microsoft variant version 3
        case AV_CODEC_ID_H263P:        // H.263+ / H.263-1998 / H.263 version 2
        case AV_CODEC_ID_HUFFYUV:      // HuffYUV
        case AV_CODEC_ID_H264:         // libx264 H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10
        case AV_CODEC_ID_FFV1:         // FFmpeg video codec #1
        case AV_CODEC_ID_ZLIB:         // LCL (LossLess Codec Library) ZLIB
        //case AV_CODEC_ID_SNOW:         // Snow
        case AV_CODEC_ID_PNG:          // PNG image
        case AV_CODEC_ID_PPM:          // PPM (Portable PixelMap) image
        case AV_CODEC_ID_PBM:          // PBM (Portable BitMap) image
        case AV_CODEC_ID_PGM:          // PGM (Portable GrayMap) image
        case AV_CODEC_ID_PGMYUV:       // PBM (Portable GrayMap YUV) image
        case AV_CODEC_ID_PAM:          // PAM (Portable AnyMap) image
        case AV_CODEC_ID_FFVHUFF:      // Huffyuv FFmpeg variant
        case AV_CODEC_ID_BMP:          // BMP image
        case AV_CODEC_ID_ZMBV:         // Zip Motion Blocks Video
        case AV_CODEC_ID_TARGA:        // Truevision Targa image
        case AV_CODEC_ID_TIFF:         // TIFF image
            extension = "avi";
        break;

        case AV_CODEC_ID_THEORA:       // libtheora Theora
            extension = "ogg";
        break;

        case AV_CODEC_ID_DVVIDEO:      // Digital Video
            extension = "dv";
        break;

        case AV_CODEC_ID_WMV1:         // Windows Media Video 7
        case AV_CODEC_ID_WMV2:         // Windows Media Video 8
            extension = "wmv";
        break;

        case AV_CODEC_ID_FLV1:         // Flash Video (FLV) / Sorenson Spark / Sorenson H.263
        case AV_CODEC_ID_SVQ1:         // Sorenson Vector Quantizer 1
        case AV_CODEC_ID_FLASHSV:      // Flash Screen Video
            extension = "flv";
        break;

        case AV_CODEC_ID_RV10:         // Real Video 1
        case AV_CODEC_ID_RV20:         // Real Video 2
            extension = "rv";
        break;

        case AV_CODEC_ID_ASV1:         // ASUS V1
        case AV_CODEC_ID_ASV2:         // ASUS V2
            extension = "asv";
        break;

        case AV_CODEC_ID_ROQ:         // id RoQ Video
            extension = "roq";
        break;

        case AV_CODEC_ID_QTRLE:       // QuickTime Animation (RLE) video
            extension = "mov";
        break;
